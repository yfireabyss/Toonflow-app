...
const vendor: VendorConfig = {
  ...
  models: [
    // ========== 仅保留三类工作流（2026-09-16 主人口径）==========
    // 视频: MiniMax-H3 x4 (fl2va-first / ref2va-image / t2va / v2a)
    // 图像: Qwen-Image x2 (t2i / edit) + FLUX2 x3 (t2i / multiref / turbo-lora)
...
async function submitAndPoll(promptId: string, prefer: "video" | "image" | "audio", pollIntervalMs: number, pollTimeoutMs: number): Promise<{ filename: string; subfolder: string; type: string }> {
  ...
  // 2026-09-16 加固: 「任务丢失」快速判定 (原文只靠 queueWaitCapMs 兜底, 最长干等 70 分钟)
  // 背景: 原逻辑「不在 queue 也不在 history 就不计时」在 ComfyUI 被重启/队列被清空时,
  //       该调用会一直空转到 pollTimeoutMs + 60min, 期间占着 ai.ts 的全局并发槽
  //       (comfyMediaLimit = pLimit(2)), 导致之后所有 comfyui 图像/视频任务全部堵在排队,
  //       一条都到不了 ComfyUI (2026-09-16 实案: 占满 2 槽 → 资产图全部卡「生成中」).
  // 规则: ① 提交后超过 submitGraceMs 仍从未出现在 queue/history → 判定提交丢失
  //       ② 曾出现过(排队/运行/历史), 之后从 queue 与 history 双双消失超过 lostGraceMs → 判定被清空
  const submitGraceMs = 120_000;
  const lostGraceMs = 45_000;
  ...
  const result = await pollTask(async () => {
    ...
    // 4) queue 与 history 都没有该 promptId → 先按窗口期处理, 超过宽限即判定任务丢失
    execStart = null;
    if (everSeen) {
      if (absentSince === null) absentSince = Date.now();
      if (Date.now() - absentSince > lostGraceMs) {
        return { completed: true, error: `ComfyUI 队列与历史中均已找不到该任务 (promptId=${promptId}, 连续消失 ${lostGraceMs}ms), 疑似 ComfyUI 重启或队列被清空, 判定任务丢失` };
      }
    } else if (Date.now() - submittedAt > submitGraceMs) {
      return { completed: true, error: `提交后 ${submitGraceMs}ms 内该任务始终未出现在 ComfyUI 队列/历史中 (promptId=${promptId}), 判定提交丢失` };
    }
    return { completed: false };
