...
**2026-08-31 优化：参考图数量上限约束（防 flux2-t2i-multiref 多参塌缩）**
`flux2-t2i-multiref` 在多张参考图约束下容易让画面构图/色调/姿态向参考图收敛，造成"多张分镜图视觉相似、md5 不同但看上去一样"。约束如下：
- **每条分镜 prompt 的 `@图N` 引用数量上限 = 2 张**（不是当前"按 associateAssetsIds 全部引用"），按真实需要选择：
  - **首选项**：**仅引用 1 张**（典型：内景就 `@图N 场景`、人像特写就 `@图N 角色`）。参考图越少，prompt 自由表达越强，分镜图差异越大。
  - **次选**：必须同时表达场景+角色时，引用 2 张（`@图1 场景 + @图2 角色`），**禁止同时引用同类型**（不要 `@图1 场景 @图2 场景`）。
  - **明确禁止**：单条 prompt 引用 3 张及以上 `@图N`（极易塌缩，且与 videoDesc 文字描述的"对应关系"过于复杂）。
- **分镜表里多于 2 张的资产**（如方舟号/年轻大副/全息星图 3 张），只挑最关键的 1-2 张标注 `@图N`，其余在 prompt 正文中用文字描述即可（flux2 文生图对纯文字描述同样响应良好）。
...
> **承接上镜（仅同场内、非该场第一行才写，置于 videoDesc 最前）**：生成本行 videoDesc 前，先通读**同一「场」内上一行**的「画面描述」与「角色动作」（并参「空间关系/朝向」），推导上镜结尾应被本镜承接的画面内容，综合为一句承接过渡，至少覆盖：①**画面/场景定格状态**——上镜结束瞬间的画面呈现（角色与关键道具的位置、姿态、正在进行的交互）；②**角色最后动作**——动作收尾后的形态（不是动作起始，而是定格时的终态）；③**位置与朝向**——角色在画面中的方位与面向。以 `承接上镜：{定态描述}` 为前缀拼入本行 `videoDesc` 开头，让本镜从该结束状态自然延续。每个「场」的第一行（含整片第一行）无上镜可承接，**跳过本段**；不得跨「场」承接（硬切换场不写承接）。
