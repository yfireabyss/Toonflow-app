...
- **承接上镜（跨镜剧情呼应，重点）**：`videoDesc` 若以「承接上镜：{定态描述}」开头，须把该定态作为提示词**承接开头**（如 `Continuing from the previous shot where {上一镜定格状态}`）置于首句或 `[Narrative]` 段开头，再展开本镜动作；本镜主体动作须是承接态的**自然延续或转折**，禁止凭空跳变；提示词结尾写明本镜动作**完成状态**，为下一镜留承接点。无「承接上镜」前缀时跳过本规则。
